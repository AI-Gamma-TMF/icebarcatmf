...
class RUG extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    if (prevProps.initialState !== this.props.initialState) {
      // Create new image objects from updated initialState
      const newImages = this.props.initialState
        .slice() // make a shallow copy
        .reverse()
        .map((item) => this.create({ done: true, ...item }))
        .reverse()
      ...
      <Context.Provider value={contextValue}>
        <div className={`rug ${className}`} style={style}>
          ...
          <input
            ...
            onChange={(event) => {
              if (!this.props.isHidden({ module: { key: 'Gallery', value: 'U' } })) {
