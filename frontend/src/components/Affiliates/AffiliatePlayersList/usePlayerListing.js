...
import { getAffiliatesPlayerList } from '../../../utils/apiCalls'
...
const usePlayerListing = () => {
  ...
  const { isLoading: loading, data: res } = useQuery({
    queryKey: ['affiliatePlayersList', limit, page, debouncedSearch, kycOptions, orderBy, sort, globalSearch],
    queryFn: ({ queryKey }) => {
            const params = {pageNo: queryKey[2], limit: queryKey[1]};
            params.affiliateCode=userDetails.affiliateCode;
      ...
      return getAffiliatesPlayerList(params)
    }
  })

  const playersData = res?.data?.data||[]
