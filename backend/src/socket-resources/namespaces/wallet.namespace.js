...
export default function (io) {
  ...
  namespace.use(async (socket, next) => {
    try {
      ...
      const accessToken = socket.handshake.headers.cookie.split('accessToken=')[1]?.split(';')[0] || socket?.handshake?.headers['x-access-token']
      if (!accessToken) {
  ...
  namespace.on('connection', (socket) => {
    ...
    socket.join(SOCKET_ROOMS.MULTI_LOGIN + ':' + socket.operator.userId)
  })
}
