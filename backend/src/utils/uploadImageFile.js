...
export async function uploadImageFile (files, affiliate) {
  ...
  try {
    ...
    for (const ele of awaitedFiles) {
      const element = {}
      const key = `${config.get('env')}/assets/${LOGICAL_ENTITY.USER_PROFILE}/${
        ele.fieldname
      }-${affiliate.affiliateId}-${Date.now()}.${
        ele.originalname.split('.')[1]
      }`
      const s3Params = {
        ACL: 'public-read',
      ...
      element.affiliateId = affiliate.affiliateId
